  ...
  const handleDeleteClick = async (PostId) => {
    ...
    try {
      const respons = await axios.delete(
        `https://localhost:44307/api/post/delete/${PostId}`
      );
      ...
      if (respons.data.success) {
        ...
        setAllProducts((prevProducts) =>
          prevProducts.filter((product) => product.PostId !== PostId)
        );
      } else {
      ...
      <div className="col-span-12 bg-[#dfe4ea] text-[#192a56] pr-8 pl-8 shadow-md">
        <div className="bg-[#dfe4ea] text-white min-h-screen ">
          <div className="container mx-auto">
            <h2 className="text-3xl font-bold mb-3 text-center text-[#192a56]">
              User Post
            </h2>
            ...
            <div className="mt-1">
              {allProducts.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 p-8 lg:grid-cols-3 gap-4">
                  {allProducts.map((product) => (
                    <div
                      key={product.PostId}
                      className="w-full md:max-w-md mx-auto  bg-white shadow-md rounded-md p-8"
                    >
                      <div className="flex items-center mb-4">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-[#192a56]">
                            {product.Name}
                          </h3>
                          <p className="text-gray-500">
                            {/* Add timestamp or date here */}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-4">
                        ...
                        <ul>
                          {comments
                            .filter(
                              (comment) => comment.postId === product.PostId
                            )
                            .map((comment, index) => (
                        ...
                        <div className="flex items-center mt-2">
                          ...
                          <button
                            onClick={() =>
                              handleAddComment(product.PostId, newComment)
                            }
